...
The **nlraa** is distributed as part of a publication that illustrates the
fit of nonlinear regression models. The old version of the package can be installed using R-forge. 
\texttt{install.packages("nlraa", repos="http://R-Forge.R-project.org")}.

The new version of the package will be in github.

## Example

We start by looking at biomass accumulation data from an experiment conducted in
Greece by Danalatos, Archontoulis and . 
...
described by Yin et al. (2003). In **nlraa** we provide **selfStart**
...
Figure **insert-label** shows a much lower bias at lower values.
...
in (Fig. **insert-label**). The amount of dispersion around zero
...
We finalize the fitting exercise by plotting observed and predicted **insert-label**.
