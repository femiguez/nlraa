...
2. To clarify, it is ok if the parameter estimates distributions deviate from normal to some extent and/or if they are correlated. It is the degree of these two things. If they strongly deviate from normality, this can represent an issue and if the correlation is very high, it might point to an over-parameterized model (but nor always!)
...
```{r, Loblolly-bootstrap-estimates}
Lob.bt.pe <- boot_nls(fnlm1) ## This takes a few seconds
pairs(Lob.bt.pe$t, labels = c("Asym", "R0", "lrc"))
print(cov2cor(var(Lob.bt.pe$t)), digits = 2) ## Correlation matrix
```

These distributions deviate somewhat from a multivariate normal. (A formal test would reject the hypothesis that the data is MVN - not shown.) Here I propose to compute confidence bands to these data using the following methods:
...
```{r, Theoph-PB-between-subjects}
pred_band_BS <- function(x) predict(x, newdata = ndat, level = 1)
fm0.Theoph.bt <- boot_nlme(fm0.Theoph, pred_band_BS)
...
In the previous calculation of prediction bands I have fixed the Does at the median value, this is part of the reason why the uncertainty seems lower than it should be. The other reason is that I'm not taking into account the uncertainty from the random effects. (I will implement this the day I needed because it is not trivial for nonlinear mixed models). For linear mixed models using the **lme** function this is indeed possible.
